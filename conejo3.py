...
async def logUserBot():
    load_dotenv()
    api_id = 29922961 # AQUI VA EL API ID
    api_hash = "78b45acfe85cb8cf6c0b8523b3e1229a" # AQUI VA EL API HASH
    phone_number = "51949877932" # AQUI VA EL NRO DE CEL JUNTO CON PREFIJO 51
    ...
    await client.send_message("@spmconejo3", f'<b>Bot encendido</b>', parse_mode="HTML") # EN "" VA EL ENLACE DEL CANAL
    spammer_group = -4559833950 #AQUI VA EL ID DEL GRUPO
    ...
    while True:
        ...
        try:
            await client.send_message("@spmconejo3", f"<b>CANTIDAD DE MENSAJES CONSEGUIDOS PARA PUBLICAR</b> <code>{len(messages_list)-1}</code>",parse_mode="HTML")
        except:
            pass
                ...
                if i['group_name'] not in ["Spam 2024","RESPALDO🇵🇪BINS PERU","➳𝒀𝑨𝑷𝑬 𝑫𝑬 𝑬𝑺𝑻𝑨𝑭𝑨𝑫𝑶𝑹𝑬𝑺 ✧","QUEMANDO ESTAFADORES","𝐏𝐄𝐑Ú 𝐀𝐘𝐔𝐃𝐀","Referencias Elmer 💸","🎭 CANAL MUNDO STREAMING PERÚ 🇵🇪","TU MARKETPLACE"]:
                    ...
                    for message_spam in messages_list:
                        ...
                        try:
                            ...
                            await client.send_message("@spmconejo3", f'<b>Error enviando mensajes a {i["group_id"]}</b> - <code>{i["group_name"]}<code>\nCausa:{error}',parse_mode="HTML")
                            resultado = False
                        if resultado:
                            await client.send_message("@spmconejo3", f'<b>Mensaje enviado a {i["group_id"]}</b> - <code>{i["group_name"]}</code>',parse_mode="HTML")  
                        await asyncio.sleep(90)
                        if j==5: break
            await client.send_message("@spmconejo3", f'<b>RONDA ACABADA</b>', parse_mode="HTML")
